...
static void do_approve(const bagl_element_t *e) {
  ...
  deriveAndSign(G_io_apdu_buffer, SCHNORR_SIG_LEN_RS, ctx->keyIndex, ctx->hash,
                SHA256_HASH_LEN);
...
UX_FLOW_DEF_NOCB(ux_signhash_flow_1_step, pnn,
                 {
                     &C_icon_certificate,
                     "Sign SHA256 Hash",
                     (char *)ctx->indexStr,
                 });
UX_FLOW_DEF_NOCB(ux_signhash_flow_2_step, bnnn_paging,
                 {
                     .title = "Hash",
                     .text = (char *)ctx->hexHash,
                 });
UX_FLOW_DEF_VALID(ux_signhash_flow_3_step, pn, do_approve(NULL),
                  {
                      &C_icon_validate_14,
                      "Sign",
                  });
UX_FLOW_DEF_VALID(ux_signhash_flow_4_step, pn, do_reject(NULL),
                  {
                      &C_icon_crossmark,
                      "Cancel",
                  });
...
ui_signHash_approve_button(unsigned int button_mask,
  ...
  case BUTTON_EVT_RELEASED | BUTTON_RIGHT: // APPROVE
    // Derive the secret key and sign the hash, storing the signature in
    // the APDU buffer.
    deriveAndSign(G_io_apdu_buffer, SCHNORR_SIG_LEN_RS, ctx->keyIndex,
                  ctx->hash, 32);
    // Send the data in the APDU buffer, which is a 64 byte signature.
    io_exchange_with_code(SW_OK, SCHNORR_SIG_LEN_RS);
    // Return to the main screen.
