...
bool pb_field_iter_begin(pb_field_iter_t *iter, const pb_field_t *fields,
                         void *dest_struct) {
...
bool pb_field_iter_next(pb_field_iter_t *iter) {
  ...
  if (prev_field->tag == 0) {
    /* Handle empty message types, where the first field is already the
     * terminator. In other cases, the iter->pos never points to the terminator.
     */
    return false;
  }
  ...
  if (((const pb_field_t *)PIC(iter->pos))->tag == 0) {
    ...
    (void)pb_field_iter_begin(iter, ((const pb_field_t *)PIC(iter->start)),
                              iter->dest_struct);
    return false;
  } else {
    ...
    if (PB_HTYPE(prev_field->type) == PB_HTYPE_ONEOF &&
        PB_HTYPE(((const pb_field_t *)PIC(iter->pos))->type) ==
            PB_HTYPE_ONEOF &&
    ...
    iter->pData = (char *)iter->pData + prev_size +
                  ((const pb_field_t *)PIC(iter->pos))->data_offset;
    iter->pSize =
        (char *)iter->pData + ((const pb_field_t *)PIC(iter->pos))->size_offset;
    return true;
  }
}
...
bool pb_field_iter_find(pb_field_iter_t *iter, uint32_t tag) {
  ...
  do {
    if (((const pb_field_t *)PIC(iter->pos))->tag == tag &&
        PB_LTYPE(((const pb_field_t *)PIC(iter->pos))->type) !=
            PB_LTYPE_EXTENSION) {
...
bool check_stack_overflow(uint16_t min_stack_gap) {
  ...
  PRINTF("+++++++Stack remaining: CUR_STACK_ADDR: %p, STACK_LIMIT: %p, "
         "Available: %d\n",
