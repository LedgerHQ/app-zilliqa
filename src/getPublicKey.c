...
static int prepareZilPubKeyAddr() {
  ...
  // 73 is the max size needed, as per bech32_addr_encode spec. 3 more for
  // "zil".
  char bech32Str[73 + 3];
  ...
  if (ctx->genAddr) {
    ...
    os_memcpy(ctx->fullStr, G_io_apdu_buffer + publicKey.W_len,
              BECH32_ADDRSTR_LEN);
    ...
    bin2hex(ctx->fullStr, sizeof(ctx->fullStr), G_io_apdu_buffer,
            publicKey.W_len);
  }
...
UX_STEP_VALID(ux_display_public_flow_3_step, pb, do_approve(NULL),
              {
                  &C_icon_validate_14,
                  "Approve",
              });
UX_STEP_VALID(ux_display_public_flow_4_step, pb, do_reject(NULL),
              {
                  &C_icon_crossmark,
                  "Reject",
              });
...
ui_prepro_getPublicKey_compare(const bagl_element_t *element) {
...
static unsigned int
ui_getPublicKey_compare_button(unsigned int button_mask,
...
static unsigned int
ui_getPublicKey_approve_button(unsigned int button_mask,
                               unsigned int button_mask_counter) {
...
void handleGetPublicKey(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,
  ...
  int n = bin64b2dec(ctx->keyStr + offset, sizeof(ctx->keyStr) - offset,
                     ctx->keyIndex);
