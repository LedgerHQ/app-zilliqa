...
static bool checkreturn buf_read(pb_istream_t *stream, pb_byte_t *buf,
                                 size_t count);
static bool checkreturn read_raw_value(pb_istream_t *stream,
                                       pb_wire_type_t wire_type, pb_byte_t *buf,
                                       size_t *size);
static bool checkreturn decode_static_field(pb_istream_t *stream,
                                            pb_wire_type_t wire_type,
                                            pb_field_iter_t *iter);
static bool checkreturn decode_callback_field(pb_istream_t *stream,
                                              pb_wire_type_t wire_type,
                                              pb_field_iter_t *iter);
static bool inline checkreturn decode_field(pb_istream_t *stream,
                                            pb_wire_type_t wire_type,
                                            pb_field_iter_t *iter)
    __attribute__((no_instrument_function));
static void iter_from_extension(pb_field_iter_t *iter,
                                pb_extension_t *extension);
static bool checkreturn default_extension_decoder(pb_istream_t *stream,
                                                  pb_extension_t *extension,
                                                  uint32_t tag,
                                                  pb_wire_type_t wire_type);
static bool checkreturn decode_extension(pb_istream_t *stream, uint32_t tag,
                                         pb_wire_type_t wire_type,
                                         pb_field_iter_t *iter);
static bool checkreturn find_extension_field(pb_field_iter_t *iter);
static void pb_field_set_to_default(pb_field_iter_t *iter);
static void pb_message_set_to_defaults(const pb_field_t fields[],
                                       void *dest_struct);
static bool checkreturn pb_dec_varint(pb_istream_t *stream,
                                      const pb_field_t *field, void *dest);
static bool inline checkreturn pb_decode_varint32_eof(pb_istream_t *stream,
...
static bool checkreturn pb_dec_svarint(pb_istream_t *stream,
                                       const pb_field_t *field, void *dest);
static bool checkreturn pb_dec_fixed32(pb_istream_t *stream,
                                       const pb_field_t *field, void *dest);
static bool checkreturn pb_dec_fixed64(pb_istream_t *stream,
                                       const pb_field_t *field, void *dest);
...
static bool checkreturn pb_dec_submessage(pb_istream_t *stream,
                                          const pb_field_t *field, void *dest);
static bool checkreturn pb_dec_fixed_length_bytes(pb_istream_t *stream,
...
static bool checkreturn allocate_field(pb_istream_t *stream, void *pData,
                                       size_t data_size, size_t array_size);
static bool checkreturn pb_release_union_field(pb_istream_t *stream,
                                               pb_field_iter_t *iter);
...
static bool checkreturn inline decode_pointer_field(pb_istream_t *stream,
                                                    pb_wire_type_t wire_type,
                                                    pb_field_iter_t *iter)
    __attribute__((no_instrument_function));
...
void __cyg_profile_func_enter(void *func, void *callsite) {
  ...
  PRINTF("-> %p '%s' [from %p], STACK %p, left: %d\n", func, fname, callsite,
         &fname,
         ((void *)&fname) -
             &_ebss); //, last_stack_left - (((void*)&fname) - &_ebss));
...
bool checkreturn pb_decode_tag(pb_istream_t *stream, pb_wire_type_t *wire_type,
...
bool checkreturn pb_close_string_substream(pb_istream_t *stream,
                                           pb_istream_t *substream) {
...
static bool checkreturn decode_static_field(pb_istream_t *stream,
  ...
  case PB_HTYPE_REQUIRED:
    return func(stream, ((const pb_field_t *)PIC(iter->pos)), iter->pData);
  ...
  case PB_HTYPE_REPEATED:
    if (wire_type == PB_WT_STRING && PB_LTYPE(type) <= PB_LTYPE_LAST_PACKABLE) {
#ifndef PROTO2_SUPPORT
      return false;
#else
      /* Packed array */
      bool status = true;
      ...
      while (substream.bytes_left > 0 &&
             *size < ((const pb_field_t *)PIC(iter->pos))->array_size) {
        void *pItem = (char *)iter->pData +
                      ((const pb_field_t *)PIC(iter->pos))->data_size * (*size);
      ...
      void *pItem = (char *)iter->pData +
                    ((const pb_field_t *)PIC(iter->pos))->data_size * (*size);
      if (*size >= ((const pb_field_t *)PIC(iter->pos))->array_size)
        PB_RETURN_ERROR(stream, "array overflow");

      (*size)++;
      return func(stream, ((const pb_field_t *)PIC(iter->pos)), pItem);
    }

  case PB_HTYPE_ONEOF:
    *(pb_size_t *)iter->pSize = ((const pb_field_t *)PIC(iter->pos))->tag;
    if (PB_LTYPE(type) == PB_LTYPE_SUBMESSAGE) {
      /* We memset to zero so that any callbacks are set to NULL.
       * Then set any default values. */
      memset(iter->pData, 0, ((const pb_field_t *)PIC(iter->pos))->data_size);
      pb_message_set_to_defaults(
          (const pb_field_t *)((const pb_field_t *)PIC(iter->pos))->ptr,
          iter->pData);
    }
    return func(stream, ((const pb_field_t *)PIC(iter->pos)), iter->pData);
...
/* Clear a newly allocated item in case it contains a pointer, or is a
 * submessage. */
static void initialize_pointer_field(void *pItem, pb_field_iter_t *iter) {
  if (PB_LTYPE(((const pb_field_t *)PIC(iter->pos))->type) == PB_LTYPE_STRING ||
      PB_LTYPE(((const pb_field_t *)PIC(iter->pos))->type) == PB_LTYPE_BYTES) {
    *(void **)pItem = NULL;
  } else if (PB_LTYPE(((const pb_field_t *)PIC(iter->pos))->type) ==
             PB_LTYPE_SUBMESSAGE) {
    /* We memset to zero so that any callbacks are set to NULL.
     * Then set any default values. */
    ...
    pb_message_set_to_defaults(
        (const pb_field_t *)((const pb_field_t *)PIC(iter->pos))->ptr, pItem);
  }
}
#endif

static bool checkreturn decode_pointer_field(pb_istream_t *stream,
                                             pb_wire_type_t wire_type,
                                             pb_field_iter_t *iter) {
  ...
  case PB_HTYPE_ONEOF:
    ...
    if (PB_LTYPE(type) == PB_LTYPE_STRING || PB_LTYPE(type) == PB_LTYPE_BYTES) {
      return func(stream, ((const pb_field_t *)PIC(iter->pos)), iter->pData);
    } else {
      if (!allocate_field(stream, iter->pData,
                          ((const pb_field_t *)PIC(iter->pos))->data_size, 1))
        return false;

      initialize_pointer_field(*(void **)iter->pData, iter);
      return func(stream, ((const pb_field_t *)PIC(iter->pos)),
                  *(void **)iter->pData);
    }
    ...
    if (wire_type == PB_WT_STRING && PB_LTYPE(type) <= PB_LTYPE_LAST_PACKABLE) {
      ...
      while (substream.bytes_left) {
        if ((size_t)*size + 1 > allocated_size) {
           ...
           * upwards. */
          allocated_size +=
              (substream.bytes_left - 1) /
                  ((const pb_field_t *)PIC(iter->pos))->data_size +
              1;
          if (!allocate_field(&substream, iter->pData,
                              ((const pb_field_t *)PIC(iter->pos))->data_size,
                              allocated_size)) {
            status = false;
            break;
      ...
      pItem = *(char **)iter->pData +
              ((const pb_field_t *)PIC(iter->pos))->data_size * (*size - 1);
      initialize_pointer_field(pItem, iter);
      return func(stream, ((const pb_field_t *)PIC(iter->pos)), pItem);
    }
...
static bool checkreturn decode_callback_field(pb_istream_t *stream,
  ...
  if (wire_type == PB_WT_STRING) {
    ...
    do {
      if (!pCallback->funcs.decode(&substream,
                                   ((const pb_field_t *)PIC(iter->pos)), arg))
    ...
    return pCallback->funcs.decode(&substream,
                                   ((const pb_field_t *)PIC(iter->pos)), arg);
  }
}
...
static bool checkreturn find_extension_field(pb_field_iter_t *iter) {
  ...
  do {
    if (PB_LTYPE(((const pb_field_t *)PIC(iter->pos))->type) ==
        PB_LTYPE_EXTENSION)
      return true;
    ...
    if (init_data) {
      if (PB_LTYPE(((const pb_field_t *)PIC(iter->pos))->type) ==
        ...
        pb_message_set_to_defaults(
            (const pb_field_t *)((const pb_field_t *)PIC(iter->pos))->ptr,
            iter->pData);
      } else if (((const pb_field_t *)PIC(iter->pos))->ptr != NULL) {
        /* Initialize to default value */
        memcpy(iter->pData, ((const pb_field_t *)PIC(iter->pos))->ptr,
               ((const pb_field_t *)PIC(iter->pos))->data_size);
      } else {
        /* Initialize to zeros */
        memset(iter->pData, 0, ((const pb_field_t *)PIC(iter->pos))->data_size);
...
bool checkreturn pb_decode_noinit(pb_istream_t *stream,
...
#ifdef PROTO2_SUPPORT
    if (PB_HTYPE(((const pb_field_t *)PIC(iter.pos))->type) ==
            PB_HTYPE_REQUIRED &&
  ...
  /* Check that all required fields were present. */
  {
    ...
    if (PB_HTYPE(last_type) == PB_HTYPE_REQUIRED &&
        ((const pb_field_t *)PIC(iter.pos))->tag != 0)
      req_field_count++;
...
bool checkreturn pb_decode(pb_istream_t *stream, const pb_field_t fields[],
                           void *dest_struct) {
  bool status;
...
bool pb_decode_delimited_noinit(pb_istream_t *stream, const pb_field_t fields[],
                                void *dest_struct) {
  pb_istream_t substream;
  bool status;
...
bool pb_decode_nullterminated(pb_istream_t *stream, const pb_field_t fields[],
                              void *dest_struct) {
...
static bool pb_release_union_field(pb_istream_t *stream,
                                   pb_field_iter_t *iter) {
  // if an error has been observed, no need to release memory
  if (stream->errmsg) {
    return false;
  ...
  pb_size_t new_tag =
      ((const pb_field_t *)PIC(iter->pos))->tag; /* New which_ value */
...
static void pb_release_single_field(const pb_field_iter_t *iter) {
  ...
  if (PB_HTYPE(type) == PB_HTYPE_ONEOF) {
    if (*(pb_size_t *)iter->pSize != ((const pb_field_t *)PIC(iter->pos))->tag)
  ...
  if (PB_LTYPE(type) == PB_LTYPE_EXTENSION) {
    ...
    if (PB_HTYPE(type) == PB_HTYPE_REPEATED) {
      ...
      if (PB_ATYPE(type) == PB_ATYPE_STATIC &&
          count > ((const pb_field_t *)PIC(iter->pos))->array_size) {
    ...
    if (pItem) {
      while (count--) {
        pb_release(
            (const pb_field_t *)((const pb_field_t *)PIC(iter->pos))->ptr,
            pItem);
        pItem = (char *)pItem + ((const pb_field_t *)PIC(iter->pos))->data_size;
      }
    }
  }
...
bool pb_decode_fixed32(pb_istream_t *stream, void *dest) {
  ...
  *(uint32_t *)dest = ((uint32_t)bytes[0] << 0) | ((uint32_t)bytes[1] << 8) |
...
bool pb_decode_fixed64(pb_istream_t *stream, void *dest) {
  ...
  *(uint64_t *)dest = ((uint64_t)bytes[0] << 0) | ((uint64_t)bytes[1] << 8) |
...
static bool checkreturn pb_dec_submessage(pb_istream_t *stream,
  ...
  bool status;
  pb_istream_t substream;
  const pb_field_t *submsg_fields = (const pb_field_t *)PIC(field->ptr);
...
static bool checkreturn pb_dec_fixed_length_bytes(pb_istream_t *stream,
  ...
  if (size == 0) {
    /* As a special case, treat empty bytes string as all zeros for
     * fixed_length_bytes. */
